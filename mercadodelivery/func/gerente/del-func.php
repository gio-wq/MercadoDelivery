...
/*VERIFICAÇÃO DE USUÁRIO EXISTENTE*/
		$checagem = mysqli_query($conn, "SELECT nome_ent FROM tbfuncionarios WHERE nome_func='$nome_func';") or die("Não existe");
		$row_check = mysqli_num_rows($checagem);
		if ($row_check == 0){
			echo "Funcionário Não Cadastrado";
		}else{
			$sql = "DELETE FROM tbfuncionarios WHERE cpf_func='$cpf_func';";
